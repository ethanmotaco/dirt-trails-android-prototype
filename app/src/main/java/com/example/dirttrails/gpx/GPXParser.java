...
public class GPXParser {
    ...
    //read inputstream into parser, return parsed GPX object
    public GPX read(InputStream in) throws XmlPullParserException, IOException {
        try {
    ...
    //Read each tag and turn data from each tag into corresponding object
    private GPX readGPX(XmlPullParser parser) throws XmlPullParserException, IOException {
        ...
        //iterate through each tag from the xml parser
        while (loopMustContinue(parser.next())) {
            ...
            //build the object for the corresponding tag
            switch (name) {
                case TAG_METADATA:
        ...
        //add end tag and finish building the GPX object
    ...
    //Check if loop must continue by checking if next is XML end tag or end of document
    ...
    /*
    *
    * Below is code for each parsing each GPX attribute into a corresponding object
    *
     */
    ...
    private String readSym(XmlPullParser parser) throws IOException, XmlPullParserException {
        return readString(parser, TAG_SYM);
    }
